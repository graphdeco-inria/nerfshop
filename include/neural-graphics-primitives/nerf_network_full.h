...
template <typename T>
class NerfNetworkFull : public NerfNetwork<T> {
public:
	using json = nlohmann::json;

	NerfNetworkFull(const json& model_config): NerfNetworkFull(3, 3, 4, 0, model_config["encoding"], model_config["dir_encoding"], model_config["network"], model_config["rgb_network"]) {}

	NerfNetworkFull(uint32_t n_pos_dims, uint32_t n_dir_dims, uint32_t n_extra_dims, uint32_t dir_offset, const json& pos_encoding, const json& dir_encoding, const json& density_network, const json& rgb_network) : m_n_pos_dims{n_pos_dims}, m_n_dir_dims{n_dir_dims}, m_dir_offset{dir_offset}, m_n_extra_dims{n_extra_dims} {
	...
	void density(cudaStream_t stream, const tcnn::GPUMatrixDynamic<float>& input, tcnn::GPUMatrixDynamic<T>& output, bool use_inference_params = true) override {
	...
	NetworkParams<T> get_rgb_params () const {
		return m_rgb_network_params;
	}

	NetworkParams<T> get_dir_encoding_params () const {
		return m_dir_encoding_params;
	}

	NetworkParams<T> get_network_params () const {
		return m_network_params;
	}

	uint32_t n_rgb_params() const {
		return m_rgb_network->n_params();
	}

	uint32_t n_dir_encoding_params() const {
		return m_dir_encoding->n_params();
	}
	...
	size_t pos_encoding_offset() const {
		return m_density_network->n_params() + m_rgb_network->n_params();
	}
	...
	uint32_t n_encoding_params() const override {
		return m_pos_encoding->n_params() + m_dir_encoding->n_params();
	}

	const std::shared_ptr<tcnn::Encoding<T>>& encoding() const override {
...
private:
	...
	NetworkParams<T> m_rgb_network_params;
	NetworkParams<T> m_dir_encoding_params;
	NetworkParams<T> m_network_params;
